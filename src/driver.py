...
def _build_options(config: dict, headless: bool) -> Options:
    options = Options()
...
def create_driver(config: dict, headless: bool = False) -> webdriver.Chrome:
    options = _build_options(config, headless)
    try:
